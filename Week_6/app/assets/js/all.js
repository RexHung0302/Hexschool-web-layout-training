...
function openCollapseHandler(that) {
    //- 取得目標
    let target = document.getElementById('navbarRow');
    //- 判斷有無 active 新增或刪除
...
function addCountNumHandler(that, num, price) {
    //- 獲取這一層的 input value
    let targetNum = $(that).parent().find('input').val();
    //- 獲取計算總價的 span
    let totalPriceSpan = $(that).parent().parent().parent().find('span')[0];
    //- 如果減少之後小於一就 return;
    if(parseInt(targetNum) + num < 1) return;
    //- 增加
...
function openShoppingCartHandler() {
    ...
    //- 購物車
    let shoppingCartModal = document.getElementById('shoppingCartModal')
    //- 判斷有無 show 新增或刪除
