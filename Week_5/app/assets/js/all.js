let userRow = document.getElementsByClassName('userRow');
...
$('#outside_reply').click(function(){
  ...
  let userRow_height = $('.userRow').height();
  //- 加入 show Class name
...
$('#cancelBtn').click(function(){
  ...
  $('#replyBox').css('height', '0');

  //- 清空輸入框框內容(使用 empty 因為是 Div 不是真的 Input)
...
$('#imgUploadInput').change(function(){
  //- 檔案
  let file = $('#imgUploadInput').prop('files')[0];
  ...
  reader.onload = function (e) {
    ...
    let nweId = new Date(); 
    // 加入 ID
    imgElement.setAttribute('id', nweId);
    // 加入圖片路徑
...
$('#showDetailBtn').click(function(){
  //- 加上 show 顯示按鈕
  $('#detailBtnInfo').toggleClass('show');
  //- 加上 show 讓箭頭跑動畫
...
$(document).mouseup(function(e){
  ...
  let clickTargetArea_2 = $('#targetTwo');   // 設定點擊目標區域
  let clickTargetArea_3 = $('#targetThree');   // 設定點擊目標區域
